resource "aws_cloudfront_distribution" "daws76" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.daws76.domain_name
        zone_id = aws_cloudfront_distribution.daws76.hosted_zone_id
      }
    }
  ]
}